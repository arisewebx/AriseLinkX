...
//           isAdmin: user.email === 'karthickrajaofficial12@gmail.com' || user.user_metadata?.role === 'admin',
//           // User status
...
// // OLD BAN FUNCTIONS (Keep for backward compatibility but we'll use the simple ones)
...
// // Delete user completely - FIXED version
// export async function deleteUser(userId) {
//   try {
//     if (!userId) throw new Error('No user ID provided');

//     // 1. First get all URLs to delete related clicks
...
//     // 2. Delete clicks in batches (if user has many URLs)
//     if (urls && urls.length > 0) {
//       const BATCH_SIZE = 50; // Prevent timeout for users with many links
//       for (let i = 0; i < urls.length; i += BATCH_SIZE) {
//         const batch = urls.slice(i, i + BATCH_SIZE);
//         const urlIds = batch.map(url => url.id);
...
//       if (deleteUrlsError) throw new Error(deleteUrlsError.message);
//     }
...
// // Delete specific link - FIXED version
...
//       .from('clicks')
//       .select('created_at, country, device, city')
//       .in('url_id', urlIds)
...
export async function deleteUserSimple(userId) {
  try {
    ...
    try {
      await supabaseAdmin.from('clicks').delete().eq('user_id', userId);
